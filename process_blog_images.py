...
使用方法：
1. 处理单个文件：python process_blog_images.py <文件路径>
2. 处理整个目录：python process_blog_images.py <目录路径>
3. 处理当前目录下的所有.md文件：python process_blog_images.py
"""
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        # 如果没有提供参数，默认处理docs目录下的所有.md文件（排除node_modules）
        docs_dir = Path('docs')
        if not docs_dir.exists():
            print("未找到docs目录")
            print("\n使用方法：")
            print("  处理单个文件: python process_blog_images.py <文件路径>")
            print("  处理整个目录: python process_blog_images.py <目录路径>")
            return
